...
/* 🔧 NEW: normalize Android / unknown mimes for Gemini */
...
router.post("/upload-chunk", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    /* 🔧 CHANGED: capture raw client mime, then derive two mimes */
    const rawMime =
    ...
    // 2) Upload buffer to Cloudinary
    /* 🔧 CHANGED: uploadAudio now returns { publicId, secureUrl } */
    ...
    rec.audio.push({
      sq: (rec.audio?.length ?? 0) + 1,
      publicId,
      secureUrl, // 🔧 CHANGED: use secureUrl (camelCase)
      sequenceId,
    });

    // 4) Write a temp file for Gemini (since generateFullTranscript expects a path)
    /* 🔧 CHANGED: give temp file a suffix to avoid ambiguous type */
    tmpPath = path.join(
      TMP_DIR,
    ...
    // 5) Transcribe + diarize this chunk
    /* 🔧 CHANGED: pass geminiMime (normalized) */
